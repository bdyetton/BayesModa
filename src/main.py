...
def infer_gss_from_model(run_name=''):
    #%% Get Z
    ...
    for fit_for in ['z', 'w', 'gss']:
        ...
        if parents[fit_for] is not None:
            _, prev_trace = pickle.load(open('../data/models/clustering_multi_epoch_w_priors_'+parents[fit_for][0]+run_name+'.pkl', 'rb'))
            ...
            if fit_for == 'gss':
                _, prev_trace = pickle.load(open('../data/models/clustering_multi_epoch_w_priors_z'+run_name+'.pkl', 'rb'))
        ...
        pickle.dump((model, trace),
                    open('../data/models/clustering_multi_epoch_w_priors_' + fit_for + run_name+'.pkl', 'wb'))
        pm.traceplot(trace)
        plt.show()


def plot_on_eeg(run_name=''):
    """Plot the data on EEG"""
    annot_map, epoch_map = pickle.load(open('../data/preprocessed/maps.pkl', 'rb'))

    model, trace = pickle.load(open('../data/models/clustering_multi_epoch_w_priors_z' + run_name + '.pkl', 'rb'))
    ...
    model, trace = pickle.load(open('../data/models/clustering_multi_epoch_w_priors_gss'+run_name+'.pkl', 'rb'))
    ...
    model, trace = pickle.load(open('../data/models/clustering_multi_epoch_w_priors_w'+run_name+'.pkl', 'rb'))
...
if __name__ == "__main__":
    ...
    #plot_wald()
    infer_gss_from_model('_multi_error')
