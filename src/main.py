...
def create_and_save_dataset(select_epochs):
    """Load dataset and save for easy model fitting"""
    file_to_load = "../data/raw/spindle_markers_by_annotator_MODA.txt"
    markers, annot_map, epoch_i_map = parse_data.load_and_parse_for_modeling(file_to_load, select_epochs)
...
def infer_gss_from_model(model_name='clustering_multi_epoch_w_priors', run_name='',
    ...
    for fit_for in ['z', 'w', 'gss']:
        ...
        print('fitting:', data_fit)
        model, steps = model_to_run(data, fit_for, data_fit)

        with model:
            trace = pm.sample(tune=1000, step=steps) # Jitter will likely break order constraints during training, so dont use
...
def plot_on_eeg(model_name='clustering_multi_epoch_w_priors', run_name='', w_null=0):
    """Plot the data on EEG, TODO - plotting when no markers found"""
    ...
    matlab_gs['theta'] = False
    matlab_gs['w'] = w_null
    matlab_gs['t'] = -1
    matlab_gs = matlab_gs.loc[matlab_gs['Epoch Num'].isin(bayes_model['Epoch Num'].unique()),:]

    markers = pd.concat([matlab_gs, raters_markers], axis=0, sort=False)
    # markers = markers.set_index('Epoch Num')

    for epoch in epoch_map:
        predicted_markers = bayes_model.loc[bayes_model['Epoch Num']==epoch,:]
...
if __name__ == "__main__":
    create_and_save_dataset(select_epochs=[51])
    infer_gss_from_model(model_name='contaminate_mixture', parents={'z': None, 'w': ('z',), 'gss': ('w')})
    plot_on_eeg(model_name='contaminate_mixture')
